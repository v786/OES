...
if(isset($_SESSION['subjChosen'])) {  // restrict users from directly jumping to this page without login
    ...
    $id = $row[0];
    $sql = "SELECT COUNT(question_id) FROM a1_question_bank WHERE subject_id = $id";
    $result = mysqli_query($GLOBALS['dbConn'], $sql);
    $row = mysqli_fetch_array($result);
    $count = $row[0];
...
?>
    ...
    function showNextQue(qNo){
            $.getJSON("quiz.json", function(nextQue){
            JSONdata = nextQue;//??
            console.log('inside');
            console.log(qNo);
        ...
        $('#quizForm').on('submit', function(){
            ...
            qNo++;
            if (qNo < num) {   /// **********************need to change it to 15 questions before submit
                showNextQue(qNo);
            } else {
                qID = quizID;
                delete quizID;
